...
begin
  ...
  values(v_company_id,auth.uid(),'update','financial_transaction',p_id,
    jsonb_build_object(
      'before', jsonb_build_object('date',v_before.created_at,'type',v_before.transaction_type,'category',v_before.category,'description',v_before.description,'status',v_before.status,'amount',v_before.amount),
      'after', jsonb_build_object('date',p_transaction_date,'type',p_transaction_type,'category',trim(p_category),'description',trim(p_description),'status',p_status,'amount',p_amount)
    ));
end;
$$;
...
begin
  ...
  if v_row.source_type <> 'manual' then
    raise exception 'Movimentações automáticas devem ser canceladas ou estornadas no módulo de origem';
  end if;
...
revoke all on function public.update_financial_transaction(uuid,date,text,text,text,text,numeric) from public,anon;
revoke all on function public.delete_financial_transaction(uuid) from public,anon;
grant execute on function public.update_financial_transaction(uuid,date,text,text,text,text,numeric) to authenticated;
