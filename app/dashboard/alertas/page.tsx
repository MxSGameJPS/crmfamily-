...
export default async function AlertsPage() {
  ...
  if (brand.key === "schemmer") {
    ...
    specialized = <section className="panel"><div className="panel-head"><h2>Assistência técnica</h2><Link href="/dashboard/assistencia" className="secondary">Abrir assistência</Link></div><div className="table-wrap"><table><thead><tr><th>OS</th><th>Cliente</th><th>Aparelho</th><th>Previsão</th><th>Alerta</th></tr></thead><tbody>
      {(orders ?? []).map((o) => <tr key={o.id}><td>#{o.order_number}</td><td>{o.customers?.name ?? "—"}</td><td>{o.device_brand} {o.device_model}</td><td>{o.estimated_delivery ? dateBR(o.estimated_delivery) : "—"}</td><td><span className={`badge ${o.status === "ready" ? "success" : o.status === "awaiting_part" ? "warning" : ""}`}>{o.status === "ready" ? "Pronto para retirada" : o.status === "awaiting_part" ? "Aguardando peça" : "Em atendimento"}</span></td></tr>)}
  ...
  if (brand.key === "housepet") {
    ...
    specialized = <section className="panel"><div className="panel-head"><h2>Agenda de hoje</h2><Link href="/dashboard/pets" className="secondary">Abrir agenda</Link></div><div className="table-wrap"><table><thead><tr><th>Hora</th><th>Pet</th><th>Tutor</th><th>Serviço</th><th>Status</th></tr></thead><tbody>
      {appointments?.map((a) => <tr key={a.id}><td>{new Intl.DateTimeFormat("pt-BR", { hour: "2-digit", minute: "2-digit", timeZone: "America/Sao_Paulo" }).format(new Date(a.scheduled_at))}</td><td><strong>{a.pets?.name ?? "—"}</strong></td><td>{a.customers?.name ?? "—"}</td><td>{a.service_type}</td><td><span className={`badge ${a.status === "ready" ? "success" : "warning"}`}>{a.status === "ready" ? "Pronto" : a.status === "confirmed" ? "Confirmado" : a.status === "in_service" ? "Em atendimento" : "Agendado"}</span></td></tr>)}
  ...
  if (brand.key === "sedux") {
    ...
    specialized = <section className="panel"><div className="panel-head"><h2>Catálogo Sedux</h2><Link href="/dashboard/catalogo" className="secondary">Abrir catálogo especial</Link></div><div className="panel-body alert-stack">
      {(batches ?? []).map((b) => <div className="alert-line warning" key={`b-${b.id}`}><strong>Validade:</strong><span>{b.products?.name ?? "Produto"} • {dateBR(b.expires_at)} • {b.quantity} un.</span></div>)}
      {lowVariants.map((v) => <div className="alert-line warning" key={`v-${v.id}`}><strong>Variação:</strong><span>{v.products?.name ?? "Produto"} • {v.sku} • estoque {v.stock_qty} / mín. {v.min_stock}</span></div>)}
  ...
  return <>
    ...
    <div className="grid-2">
      ...
      <section className="panel"><div className="panel-head"><h2>Financeiro e cobranças</h2><Link href="/dashboard/devedores" className="secondary">Ver devedores</Link></div><div className="panel-body alert-stack">
        {overdueReceivables.map((r) => <div className="alert-line danger" key={`r-${r.id}`}><strong>{r.customers?.name ?? "Cliente"}</strong><span>Venceu em {dateBR(r.due_date)} • {brl(Number(r.amount_total)-Number(r.amount_paid))}</span></div>)}
