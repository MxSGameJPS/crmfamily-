...
export default async function AccessPage() {
  ...
  const [{ data: companies }, { data: profiles }] = await Promise.all([
    ...
    supabase.from("profiles").select("id,full_name,role,company_id,companies(name)").in("role", ["store_admin","store_user"]).order("full_name"),
  ]);
  return <>
    <PageHeader eyebrow="SUPERADMIN" title="Acessos das lojas" description="Crie logins e gere novas senhas sem acessar ou alterar os dados operacionais das empresas." />
