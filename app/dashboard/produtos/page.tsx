...
export default async function ProductsPage() {
  ...
  return <>
    ...
    <section className="panel section-gap"><div className="panel-head"><h2>Produtos ativos</h2><span className="badge">{products?.length ?? 0}</span></div><div className="table-wrap"><table><thead><tr><th>Produto</th><th>SKU</th><th>Categoria</th><th>Custo</th><th>Venda</th><th>Estoque</th><th>Status</th><th></th></tr></thead><tbody>
      {products?.map((p) => { const low = Number(p.stock_qty) <= Number(p.min_stock); return <tr key={p.id}><td><strong>{p.name}</strong></td><td>{p.sku}</td><td>{p.category ?? "—"}</td><td>{brl(p.cost)}</td><td className="amount">{brl(p.price)}</td><td>{p.stock_qty}</td><td><span className={`badge ${low ? "warning" : "success"}`}>{low ? "Estoque baixo" : "Normal"}</span></td><td><div className="toolbar"><a className="secondary" href={`/dashboard/produtos/${p.id}`}>Editar</a><form action={archiveProduct}><input type="hidden" name="id" value={p.id}/><button className="danger">Arquivar</button></form></div></td></tr>; })}
